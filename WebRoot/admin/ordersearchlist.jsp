      ...
      <table class="table table-striped b-t b-light">
        ...
        <c:forEach items="${list}" var="order" varStatus="status">
          <tr>
			...
			<td><font style="font-weight: bold;">地址:&nbsp;&nbsp;</font>${order.address.name}&nbsp;&nbsp;&nbsp;&nbsp;<font style="font-weight: bold;">${order.address.addr}&nbsp;&nbsp;&nbsp;&nbsp;${order.address.tel}</font></td>
			<td>${order.fkstatus}</td>
			<td>${order.shstatus}</td>
            <td>
            <c:if test="${order.shstatus eq '待发货'}">
              <a href="faHuo.do?id=${order.id}" title="发货" class="active" ui-toggle-class=""><i class="fa fa-check text-success text-active"></i></a>
            </c:if>
            </td>
          </tr>
        </c:forEach>
